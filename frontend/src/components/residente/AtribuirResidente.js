...
function AtribuirResidente({ quartoId }) {
    ...
    const handleAtribuir = async () => {
        setLoading(true);
        try {
            await AuthService.authenticatedRequest('post', 'relatorios', `/quartos/${quartoId}/atribuir/`, {
                residenteId: residenteSelecionado,
            });
            setMensagem('Residente atribuído com sucesso.');
            setTipoMensagem('success');
            // Opcional: Recarregar a lista de residentes ou quartos após a atribuição
        } catch (error) {
            console.error('Erro ao atribuir residente:', error.response?.data);
            setMensagem(error.response?.data?.message || 'Erro ao atribuir residente.');
            setTipoMensagem('error');
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="p-4 max-w-md mx-auto mt-8">
            <div className="bg-white rounded-lg shadow-md p-6">
                <h2 className="text-xl font-bold mb-4 text-blue-600">Atribuir um Quarto ao Residente</h2>
                {mensagem && (
                    <div
...
export default AtribuirResidente;